...
// max open-time = 1393661800 (also close time)
// delta = 53139900 s = 885665 min = 14761.08 h = 615.05 days = 87.9 weeks = 20.5 months

#define N_HOURS 14763
#define N_DAYS  617

// All records = 199,688,850
...
const bool LIMITED_SAMPLE = false; // for testing only

const TString FNAME_XTRA("far");
const TString FNAME_PFX = TString::Format("/bar/xrdmon-%s-merged/xm%s", FNAME_XTRA.Data(), FNAME_XTRA.Data());

enum EAccessType
{
  AT_nocut,     // no cut on access type
  AT_local,     // server and client at UW
  AT_nonlocal,  // server UW, client elsewhere
  AT_remote,    // server elsewhere, client at UW,
...
{
  { "r1_full_loc",    "uw1/full_local.root",    AT_local,    TC_none, false }, // W->W  56,528,302
  { "r1_full_nonloc", "uw1/full_nonlocal.root", AT_nonlocal, TC_none, false }, // W->X   1,009,351
  { "r1_full_remote", "uw1/full_remote.root",   AT_remote,   TC_none, false }, // X->W     855,761

  { "r2_full_loc",    "uw2/full_local.root",    AT_local,    TC_keep_long,  false }, // W->W  17,108,892
  { "r2_full_nonloc", "uw2/full_nonlocal.root", AT_nonlocal, TC_keep_long,  false }, // W->X     694,696
  { "r2_full_remote", "uw2/full_remote.root",   AT_remote,   TC_keep_long,  false }, // X->W     702,403

  { "us1_all_n_lazy", "us1/all_n_lazy.root", AT_US, TC_none,       false }, // all w/ preload   6,310,605
  { "us1_short",      "us1/short.root",      AT_US, TC_keep_short, true  }, // short, t < 100s    703,793
  { "us1_long",       "us1/long.root",       AT_US, TC_keep_long,  true  }, // long,  t > 100s  4,534,612

  { "uw3_all_n_lazy", "uw3/all_n_lazy.root", AT_local, TC_none,       false }, // all w/ preload   56,528,278
  { "uw3_short",      "uw3/short.root",      AT_local, TC_keep_short, true  }, // short, t < 100s  35,672,599
  { "uw3_long",       "uw3/long.root",       AT_local, TC_keep_long,  true  }, // long,  t > 100s   7,839,134
};
...
void init_input()
{
  if ( ! LIMITED_SAMPLE)
  {
    TChain *t = new TChain("XrdFar");
    TString host = gSystem->HostName();
    if (host == "black.woof.org")
    {
      printf("Running on black, using reduced dataset!\n");
      t->Add(FNAME_PFX + "-2013-*.root");
    }
    else
    {
      t->Add(FNAME_PFX + "-*.root");
    }
    theTree = t;
  }
  else
  {
    TFile *f = TFile::Open(FNAME_PFX + "-2013-01.root");
    TTree *t = (TTree *) f->Get("XrdFar");
    theTree = t;
  }

  theTree->SetBranchAddress("F.", &fp);
  theTree->SetBranchAddress("U.", &up);
  theTree->SetBranchAddress("S.", &sp);
}
...
void loop_etc()
{
  ...
  for (Long64_t i = 0; i < N; ++i)
  {
    ...
    {
      case AT_local:
        if ( ! S.mDomain.EndsWith("wisc.edu") || ! U.mFromDomain.EndsWith("wisc.edu"))
          continue;
        break;
      case AT_nonlocal:
        if ( ! S.mDomain.EndsWith("wisc.edu") || U.mFromDomain.EndsWith("wisc.edu"))
    ...
    // Filter out Alja and me, and Carl and Brian. Sciaba?
    if (U.mRealName.Contains("Tadel") || U.mRealName.Contains("Vuosalo") ||
